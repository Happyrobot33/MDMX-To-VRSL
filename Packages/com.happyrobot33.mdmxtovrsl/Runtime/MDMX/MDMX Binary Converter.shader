    ...
    SubShader
    {
        ...
        Pass
        {
                ...
                uint b3, uint b4, uint b5)
            {
                ...
                {
                    //Forward loop to fix issue on quest where the original backwards loop would crash
                    for (int bit = 0; bit < 8; ++bit)
                    {
                        uint inBit = (data[idx] >> (7 - bit)) & 1u;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // if (getChannel(i.uv) == round(_Test - 1)) {
                //     col = float4(1,0,0,1);
                // }
                ...
                if (discardFlag || crcCheck != values[6]) {
                    col = tex2Dlod(_SelfTexture2D, float4(i.uv,0,0));
                    col.gb = 0;
                }
